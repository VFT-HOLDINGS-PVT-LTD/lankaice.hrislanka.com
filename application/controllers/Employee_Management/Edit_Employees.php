...
class Edit_Employees extends CI_Controller {
    ...
    public function index() {
        ...
        $data['data_sgroup'] = $this->Db_model->getData('id,super_gname', 'tbl_super_group');
    ...
    public function edit($id) {
        ...
        $data['data_set'] = $this->Db_model->getfilteredData("SELECT 
                                                                    ...
                                                                    tbl_empmaster.pName,
                                                                    tbl_empmaster.rPerson,
                                                                    tbl_empmaster.catCode,
                                                                    tbl_empmaster.Is_mApp,
                                                                    tbl_empmaster.payment_Type,
                                                                    tbl_empmaster.Comp_Num,
                                                                ...
                                                                FROM
                                                                    ...
                                                                    tbl_emp_group ON tbl_empmaster.Grp_ID = tbl_emp_group.Grp_ID
                                                                        LEFT JOIN
                                                                    tbl_super_group ON tbl_empmaster.SupGrp_ID = tbl_super_group.id
                                                                        LEFT JOIN
...
//            var_dump($data);
        ...
        $MobileAppsatus = $this->input->post('mobileApp_status');
        // echo $satus;
        $st2 = '';
        if($MobileAppsatus == 'Active'){
            $st2 = "1";
        }else{
            $st2 = "0";
        }

        $catCode = $this->input->post('cmb_emp_catCode');
        $payment_Type = $this->input->post('cmb_Payment_type');

        $data = array(
            'Company_No' => $this->input->post('companyNo'),
