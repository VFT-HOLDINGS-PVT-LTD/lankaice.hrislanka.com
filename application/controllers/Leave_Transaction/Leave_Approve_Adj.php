...
FROM
    ...
    INNER JOIN tbl_branches ON em.B_id = tbl_branches.B_id
    left JOIN tbl_emp_group ON tbl_emp_group.Sup_ID = em.Emp_Full_Name
WHERE
    le.Is_Cancel = 1 {$filter} OR Is_delete =1 AND em.EmpNo = '00001554'
  {$filter}");
...
//                    var_dump($Balance_Usd);die;
    ...
    public function ajax_StatusDelete($id)
    {
        // echo $id;
        $currentUser = $this->session->userdata('login_user');
        $Emp = $currentUser[0]->EmpNo;

        $EmpDataLeave = $this->Db_model->getfilteredDelete("DELETE FROM tbl_leave_entry WHERE LV_ID = '$id';");
