...
FROM
    ...
    public function Export_Excel()
    {
        ...
        if ($rept_type == "AttSum") {
            ...
            $extraCondition = "ir.InTime > ADDTIME(tbl_shifts.FromTime, SEC_TO_TIME(tbl_emp_group.GracePeriod * 60))";
            $where = empty($filter) ? "WHERE $extraCondition" : "$filter AND $extraCondition";
            $data_set = $this->Db_model->getfilteredData1("
                SELECT ir.EmpNo, Emp.Emp_Full_Name, ir.FDate, ir.InTime, ir.LateM, tbl_shifts.FromTime,
                    tbl_emp_group.GracePeriod,
                    ADDTIME(tbl_shifts.FromTime, SEC_TO_TIME(tbl_emp_group.GracePeriod * 60)) AS TotalShiftTime
                FROM tbl_individual_roster ir
                LEFT JOIN tbl_empmaster Emp ON Emp.EmpNo = ir.EmpNo
                INNER JOIN tbl_shifts ON ir.ShiftCode = tbl_shifts.ShiftCode
                INNER JOIN tbl_emp_group ON Emp.Grp_ID = tbl_emp_group.Grp_ID
                $where
            ")->result_array();
            ...
            $mapColumns = fn($row) => [
                ...
                $row['TotalShiftTime']
                    ...
                    $where = empty($filter) ? "WHERE iro.DayStatus = 'MS'" : "$filter AND iro.DayStatus = 'MS'";
                    $data_set = $this->Db_model->getfilteredData1("
                SELECT iro.EmpNo, Emp.Emp_Full_Name, iro.FDate, iro.InTime, iro.OutTime, iro.DayStatus
                FROM tbl_individual_roster iro
                LEFT JOIN tbl_empmaster Emp ON Emp.EmpNo = iro.EmpNo
                $where
            ")->result_array();
            ...
            $mapColumns = fn($row) => [
                ...
                $row['DayStatus']
                    ...
                    $where = "WHERE tbl_leave_entry.Is_Cancel = 0";
                    if (!empty($emp)) {
                        $where .= " AND tbl_leave_entry.EmpNo = '$emp'";
                    }
                    if (!empty($from_date) && !empty($to_date)) {
                        $where .= " AND tbl_leave_entry.Leave_Date BETWEEN '$from_date' AND '$to_date'";
                    }

                    $data_set = $this->Db_model->getfilteredData1("
                SELECT tbl_empmaster.EmpNo, tbl_empmaster.Emp_Full_Name, tbl_leave_types.leave_name,
                    tbl_leave_entry.Leave_Date, tbl_leave_entry.Leave_Count, tbl_leave_entry.Reason,
                    approved_emp.Emp_Full_Name AS Approved_By
                FROM tbl_leave_entry
                INNER JOIN tbl_leave_types ON tbl_leave_types.Lv_T_ID = tbl_leave_entry.Lv_T_ID
                INNER JOIN tbl_empmaster ON tbl_empmaster.EmpNo = tbl_leave_entry.EmpNo
                INNER JOIN tbl_empmaster AS approved_emp ON tbl_leave_entry.Approved_by = approved_emp.EmpNo
                $where
            ")->result_array();
            ...
            $mapColumns = fn($row) => [
                ...
                $row['Approved_By']
                    ...
                    $where = "WHERE sl.Is_Approve = 1";
                    if (!empty($emp)) {
                        $where .= " AND sl.EmpNo = '$emp'";
                    }
                    if (!empty($from_date) && !empty($to_date)) {
                        $where .= " AND sl.Date BETWEEN '$from_date' AND '$to_date'";
                    }

                    $data_set = $this->Db_model->getfilteredData1("
                SELECT Emp.EmpNo, Emp.Emp_Full_Name, sl.Date, sl.Reason
                FROM tbl_shortlive sl
                LEFT JOIN tbl_empmaster Emp ON Emp.EmpNo = sl.EmpNo
                $where
            ")->result_array();
            ...
            $mapColumns = fn($row) => [
                ...
                $row['Reason']
                    ...
                    $data_set = $this->Db_model->getfilteredData1("
                SELECT ir.EmpNo, Emp.Emp_Full_Name, ir.FDate, ir.DayStatus
                FROM tbl_individual_roster ir
                LEFT JOIN tbl_empmaster Emp ON Emp.EmpNo = ir.EmpNo
                $where
                GROUP BY ir.FDate, Emp.EmpNo
            ...
            $mapColumns = fn($row) => [
                ...
                $row['DayStatus']
                    ...
                    $where = empty($filter) ? "WHERE ir.DayStatus = 'PR'" : "$filter AND ir.DayStatus = 'PR'";
                    $data_set = $this->Db_model->getfilteredData1("
                SELECT ir.EmpNo, Emp.Emp_Full_Name, ir.FDate, ir.InTime, ir.OutTime
                FROM tbl_individual_roster ir
                LEFT JOIN tbl_empmaster Emp ON Emp.EmpNo = ir.EmpNo
                $where
            ")->result_array();
            ...
            $mapColumns = fn($row) => [
                ...
                $row['OutTime']
                    ...
                    $where = empty($filter) ? "WHERE ir.DayStatus = 'AB'" : "$filter AND ir.DayStatus = 'AB'";
                    $data_set = $this->Db_model->getfilteredData1("
                SELECT ir.EmpNo, Emp.Emp_Full_Name, ir.FDate, ir.DayStatus
                FROM tbl_individual_roster ir
                LEFT JOIN tbl_empmaster Emp ON Emp.EmpNo = ir.EmpNo
            ...
            $where = "";
            if (!empty($emp)) {
                $where .= " WHERE tbl_leave_allocation.EmpNo = '$emp'";
            }

            $data_set = $this->Db_model->getfilteredData1("
                SELECT tbl_empmaster.EmpNo, tbl_empmaster.Emp_Full_Name, tbl_leave_types.leave_name,
                    tbl_leave_allocation.Year, tbl_leave_allocation.Entitle,
                    tbl_leave_allocation.Used, tbl_leave_allocation.Balance
            ...
            $where = "";
            if (!empty($emp)) {
                $where .= "WHERE ir.Enroll_No = '$emp'";
            }

                    $data_set = $this->Db_model->getfilteredData1("
                SELECT Emp.EmpNo, Emp.Emp_Full_Name, ir.AttDate, ir.AttTime, ir.Status
                FROM tbl_u_attendancedata ir
    ...
    if (!empty($data_set)) {
        ...
        $sheet = $spreadsheet->getActiveSheet();
        $sheet->setTitle($rept_type . ' Report');
