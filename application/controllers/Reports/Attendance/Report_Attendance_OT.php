...
class Report_Attendance_OT extends CI_Controller {
   ...
   public function Attendance_Report_By_Cat() {
        ...
        $PaymentType = $this->input->post("cmb_Payment_type");
        ...
        if (($this->input->post("cmb_Payment_type"))) {
            if ($filter == null) {
                $filter = " where Emp.payment_Type  ='$PaymentType'";
            } else {
                $filter .= " AND Emp.payment_Type  ='$PaymentType'";
            }
        }
